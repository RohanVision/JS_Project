...
$(document).ready( () => {
    ...
    {        
        ...
        filteredTasks.forEach((task,index)=>{
            ...
            task.priority === "high" ? "#dd3737" :
            task.priority === "medium" ? "#ffc71e" :
            task.priority === "low" ? "#05aa4e" :
            "" }`;
    ...
    {        
        ...
        else if(!namePattern.test(taskName)) //validate using regex
        {
        ...
        else if(!namePattern.test(description)) //validate using regex
        {
            $("#taskDescription").next().text("*Description must contain only letters.");
            isValid = false;
        }
        else 
        {
        ...
        else if(!namePattern.test(assignedTo)) //validate using regex
        {
            $("#assignedTo").next().text("*Assigned to must contain only letters.");
            isValid = false;
        }
        else 
        {
        ...
        if (isValid) 
        {
            ...
            { 
                ...
                if(checkDuplicate(taskName))
                {
                    ...
                    {
                        tasks[editIndex] = { taskName, description, assignedTo, dueDate, priority, status };
            ...
            else 
            {
                ...
                {
                    tasks.push({ taskName, description, assignedTo, dueDate, priority, status }); // Add a new task.
                    clearInputFields(); // Clear input fields.
                }
    ...
    function filterTasks(filterText) 
    {
        return tasks.filter(task => {
            for (const key in task) {
                if (task[key].toLowerCase().includes(filterText.toLowerCase())) {
    ...
    searchFilter.addEventListener("input", () => renderTasks(filterTasks(searchFilter.value)));
